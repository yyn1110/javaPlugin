...
func main() {
	...
	if err = os.MkdirAll(g_modelPath, 0777); err != nil {
		fmt.Println("Create folder", g_modelPath, "error:", err.Error())
		os.Exit(-1)
	}
	...
	if err = os.MkdirAll(g_daoPath, 0777); err != nil {
		fmt.Println("Create folder", g_daoPath, "error:", err.Error())
		os.Exit(-1)
	}
	...
	if err = os.MkdirAll(g_dataSourcePath, 0777); err != nil {
		fmt.Println("Create folder", g_dataSourcePath, "error:", err.Error())
		os.Exit(-1)
	}
	...
	if err != nil {
		fmt.Println("Connect database:", dbURL, "Error:", err.Error())
		os.Exit(-1)
	}
	defer dbConn.Close()
	...
	case <-ch:
		...
		os.Exit(-1)
	case <-finishChannel:
		fmt.Println("Convert finished successfully!")
	}
}
...
func run(dbConn *sql.DB, wg *sync.WaitGroup) {
	...
	if rows, err = dbConn.Query("SHOW tables;"); err != nil {
		fmt.Println("SHOW tables err:", err.Error())
		return
	}
	defer rows.Close()
...
func parseTable(dbConn *sql.DB, tableName string, wg *sync.WaitGroup) {
	...
	for rows.Next() {
		...
		if strings.HasPrefix(tds.DbTypeString, "varchar") ||  strings.HasPrefix(tds.DbTypeString, "tinytext") ||  strings.HasPrefix(tds.DbTypeString, "longtext"){
			...
			tds.TypeString = "String"
			tds.TestValue = fmt.Sprintf("\"%s\"", getMaxLength(tds.FieldName, tds.DbTypeString))
		} else if strings.HasPrefix(tds.DbTypeString, "mediumtext") {
			tds.Type = FIELD_TYPE_STRING
			tds.TypeString = "String"
			tds.TestValue = fmt.Sprintf("\"%s\"", tds.FieldName)
			...
			fmt.Printf("%s|%s|%s|odd type: BigDecimal", tableName, class.ClassName, tds.DbFieldName)
			...
			fmt.Printf("%s|%s|%s|odd type: timestamp\n", tableName, class.ClassName, tds.DbFieldName)
		} else if strings.HasPrefix(tds.DbTypeString, "date") {
			tds.Type = FIELD_TYPE_TIMESTAMP
			tds.TypeString = "java.util.Date"
			tds.TestValue = "new java.util.Date()"
			fmt.Printf("%s|%s|%s|odd type: %s\n", tableName, class.ClassName, tds.DbFieldName, tds.DbTypeString)
		...
		if len(class.UnionKeys) == 1 {
			...
			if rowsCreateTable.Next() {
				...
				if err = rowsCreateTable.Scan(&t1, &t2); err != nil {
					fmt.Println("Error parse create table")
					return
				}
				...
				fmt.Println("create table no respones")
			}
			if len(class.UnionKeys) == 1 {
				class.PrimaryKey = class.UnionKeys[0]
				class.UnionKeys = nil
				fmt.Printf("%s|%s|%s|odd key: Not Primary Key\n", tableName, class.ClassName, class.PrimaryKey.DbFieldName)
				//fmt.Printf("table %s (class %s) only one key %s\n", class.TableName, class.ClassName, class.PrimaryKey.DbFieldName)
			} else {
				fmt.Printf("%s|%s|", tableName, class.ClassName)
			...
			fmt.Println("|odd key: Multi Key")
		}
		if class.PrimaryKey == nil && len(class.UnionKeys) == 0 {
			fmt.Printf("%s|%s||odd key: No Key\n", tableName, class.ClassName)
		}
	}
	if class.PrimaryKey != nil {
		if class.PrimaryKey.Type != FIELD_TYPE_INTEGER {
			fmt.Printf("%s|%s|%s|odd key: Key type is %s\n", tableName, class.ClassName, class.PrimaryKey.FieldName, class.PrimaryKey.TypeString)
		} else if !class.PrimaryKey.AutoIncrement {
			fmt.Printf("%s|%s|%s|odd key: not auto increment\n", tableName, class.ClassName, class.PrimaryKey.FieldName)
		}
	}
	/*
...
func getMaxLength(name, varcharString string) (result string) {
	...
	if length, err = strconv.Atoi(ss[0]); err != nil {
		fmt.Println("getMaxLength(", name, ", ", varcharString, ", error:", err.Error())
		return
	}
	if len(name) < length {
		result = name
...
func composeClassFile(class *classDefine) {
	...
	if file, err = os.Create(filepath.Join(g_modelPath, class.ClassName+".java")); err != nil {
		fmt.Println(filepath.Join(g_modelPath, class.ClassName+".java"), ", error:", err.Error())
		return
	}
...
func composeTestException(){
	...
	if file, err = os.Create(filepath.Join(g_testExceptionPath,"UnitTestException.java")); err != nil {
		fmt.Println("Create file", filepath.Join( g_dataSourcePath+"UnitTestException.java"), ", error:", err.Error())
		return
	}
...
func composeDataSourceFiles() {
	...
	if dynamicFile, err = os.Create(filepath.Join( g_dataSourcePath,"DynamicDataSource.java")); err != nil {
		fmt.Println("Create file", filepath.Join( g_dataSourcePath+"DynamicDataSource.java"), ", error:", err.Error())
		return
	}
	...
	if dynamicAspectFile, err = os.Create(filepath.Join( g_dataSourcePath,"DynamicDataSourceAspect.java")); err != nil {
		fmt.Println("Create file", filepath.Join(g_dataSourcePath+"DynamicDataSourceAspect.java"), ", error:", err.Error())
		return
	}
...
func composeDaoFiles(class *classDefine) {
	...
	if file, err = os.Create(filepath.Join(g_daoPath, class.ClassName+"Dao.java")); err != nil {
		fmt.Println("Create file", filepath.Join(g_daoPath, class.ClassName+"Dao.java"), ", error:", err.Error())
		return
	}
...
func composeMappingFiles(class *classDefine) {
	...
	if file, err = os.Create(filepath.Join(g_myBatisPath, class.CamelCaseName+"SqlMap.xml")); err != nil {
		fmt.Println("Create file", filepath.Join(g_myBatisPath, class.CamelCaseName+"SqlMap.xml"), ", error:", err.Error())
		return
	}
...
func composeTestFiles(class *classDefine) {
	...
	if file, err = os.Create(filepath.Join(g_testPath, class.ClassName+"Test.java")); err != nil {
		fmt.Println("Create file", filepath.Join(g_testPath, class.ClassName+"Test.java"), ", error:", err.Error())
		return
	}
...
func writeClassFields(bw *bufio.Writer, class *classDefine) {
	...
	for _, fieldName := range class.Names {
		fmt.Println(class.ClassName + "::" + fieldName)
...
func writeMappingBody(bw *bufio.Writer, class *classDefine) {
	...
	if class.PrimaryKey != nil {
		...
		default:
			fmt.Println("Unsupport primary key type:", class.PrimaryKey.TypeString)
			os.Exit(-1)
		}
...
func (res *resources) init() {
	...
	if res.daoConfigFile, err = os.Create(filepath.Join(g_testResourcesPath, (*dbName)+"-daoConfig.xml")); err != nil {
		fmt.Println("Create file", filepath.Join(g_testResourcesPath, (*dbName)+"-daoConfig.xml"), ", error:", err.Error())
		return
	}
...
func writeLog4j() {
	...
	if file, err = os.Create(filepath.Join(g_testResourcesPath, "log4j.xml")); err != nil {
		fmt.Println("Create file", filepath.Join(g_testResourcesPath, "log4j.xml"), ", error:", err.Error())
		return
	}
	defer file.Close()
	bw := bufio.NewWriter(file)
...
func writeConfigProperties() {
	...
	if file, err = os.Create(filepath.Join(g_testResourcesPath, (*dbName)+"-db.properties")); err != nil {
		fmt.Println("Create file", filepath.Join(g_testResourcesPath, (*dbName)+"-db.properties"), ", error:", err.Error())
		return
	}
	defer file.Close()
	bw := bufio.NewWriter(file)
...
func writeAbstractTest() {
	...
	if file, err = os.Create(filepath.Join(g_testPath, "AbstractTest.java")); err != nil {
		fmt.Println("Create file", filepath.Join(g_testPath, "AbstractTest.java"), ", error:", err.Error())
		return
	}
	defer file.Close()
	bw := bufio.NewWriter(file)
...
func writeConstants() {
	...
	if file, err = os.Create(filepath.Join(g_daoPath, "DataSourceConstants.java")); err != nil {
		fmt.Println("Create file", filepath.Join(g_daoPath, "DataSourceConstants.java"), ", error:", err.Error())
		return
	}
	defer file.Close()
	bw := bufio.NewWriter(file)
