...
import com.yao.yz.kubauser.persistence.dao.EcuserCustomerAddDao;
import com.yao.yz.kubauser.exception.UnitTestException;
import org.apache.log4j.Logger;
import org.junit.Test;
import org.springframework.beans.factory.annotation.Autowired;

public class EcuserCustomerAddTest extends AbstractTest {
	private static final Logger LOGGER = Logger.getLogger(EcuserCustomerAdd.class);
	@Autowired
	private EcuserCustomerAddDao dao;

	private void setObjVal(EcuserCustomerAdd sObj) {
		sObj.setEcUId(1);
	...
	public void testCase() throws UnitTestException {
		EcuserCustomerAdd objInsert = new EcuserCustomerAdd();
		setObjVal(objInsert);

		LOGGER.info("+ [EcuserCustomerAdd]");
		dao.insert(objInsert);
		Integer key = objInsert.getEcUserId();
		if (key == null || key == 0) {
			throw new UnitTestException("EcuserCustomerAddTest", "test of insert is failed");
		}
		LOGGER.info("	insert OK");

		EcuserCustomerAdd objSelect = dao.getEcuserCustomerAddByKey(key);
		if (objSelect == null) {
			throw new UnitTestException("EcuserCustomerAddTest", "test of select is failed");
		}
		LOGGER.info("	select OK");

		Integer res = dao.update(objSelect);
		if (res == null || res == 0) {
			throw new UnitTestException("EcuserCustomerAddTest", "test of update is failed");
		}
		LOGGER.info("	update OK");

		Integer del = dao.delete(key);
		if (del == null || del == 0) {
			throw new UnitTestException("EcuserCustomerAddTest", "test of delete is failed");
		}
